...
def main(config, args):
    ...
    if is_multi_class:
        ...
        logger.info(f"Using single-class dataset: {getattr(anom_dataset, 'category', 'oil')}")
        anom_loader = [
            DataLoader(
                anom_dataset,
                batch_size=MAX_BATCH_SIZE,
    ...
    if config['backbone']['model_type'] == 'none':
        img_size = config['data'].get('img_size', 8)
        target_channels = config['diffusion'].get('target_channels', 1)
        diff_in_sh = (target_channels, img_size, img_size)
    else:
...
def evaluate_inv(denoiser, feature_extractor, anom_loaders, normal_loaders, config, in_sh, epoch, eval_step, device, args):
    ...
    for normal_loader, anom_loader in zip(normal_loaders, anom_loaders):
        ...
        samples_dict = {}
        category = config["data"].get("category", getattr(anom_loader.dataset, 'category', 'unknown'))
        ...
        for batch in tqdm(normal_loader, total=len(normal_loader)):
            ...
            current_start_t = torch.zeros(bs, device=device, dtype=torch.long)

            # BEZPIECZNE POBIERANIE DANYCH
            paths = batch.get("filenames", [f"normal_{c + i}" for i in range(bs)])
            labels = batch.get("clslabels", torch.zeros(bs, dtype=torch.long)).to(device)
            anom_labels = batch.get("labels", torch.zeros(bs, dtype=torch.long)).to(device)

            if "masks" in batch:
                mask_data = batch["masks"].to(device)
            else:
                # Dla danych normalnych tworzymy maskę samych zer (wszystko OK)
                mask_data = torch.zeros((bs, 1, org_h, org_w), device=device, dtype=torch.float32)

                # KLUCZOWE: Przenosimy na CPU przed dodaniem do listy, żeby oszczędzać VRAM
            normal_gt_masks.append(mask_data.cpu())

            s_time = time.perf_counter()
            features, _ = feature_extractor(images)
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                latents_last = eval_denoiser.ddim_reverse_sample(
                    features, current_start_t, labels, eta=0.0
                )
            latents_last_l2 = torch.sum(latents_last ** 2, dim=1).sqrt()

            # Statystyki różnic
            flat_l2 = latents_last_l2.view(bs, -1)
            min_diffs_spatial = flat_l2.min(dim=1)[0]
            max_diffs_spatial = flat_l2.max(dim=1)[0]
            ...
            # FIX WYMIARÓW: unsqueeze(1) zamiast unsqueeze(0) dla poprawnej interpolacji batcha
            normal_map = F.interpolate(latents_last_l2.unsqueeze(1), size=(org_h, org_w), mode='bilinear',
            ...
            memory_meter.update(torch.cuda.max_memory_allocated() / (1024 * 1024), n=bs)

            # Zapis do wizualizacji
            org_imgs = convert2image(denormalize(images.cpu()))
            current_maps = normal_map.cpu().numpy()
            for i in range(len(org_imgs)):
                samples_dict[category][paths[i]] = {
                    "image": org_imgs[i],
                    "anomaly_map": current_maps[i],
                    "gt_mask": mask_data[i].cpu().numpy()
                }
            c += bs
        ...
        for batch in tqdm(anom_loader, total=len(anom_loader)):
            ...
            paths = batch.get("filenames", [f"anomaly_{anom_counter + i}" for i in range(bs)])
            ...
            if "masks" in batch:
                ...
                mask_data = torch.ones((bs, 1, org_h, org_w), device=device, dtype=torch.float32)

            anomaly_gt_masks.append(mask_data.cpu())
            ...
            # FIX WYMIARÓW: unsqueeze(1)
            anomaly_map = F.interpolate(latents_last_l2.unsqueeze(1), size=(org_h, org_w), mode='bilinear',
                                        align_corners=False).squeeze(1)
            ...
            org_imgs = convert2image(denormalize(images.cpu()))
            current_maps = anomaly_map.cpu().numpy()
            for i in range(len(org_imgs)):
                samples_dict[category][paths[i]] = {
                    "image": org_imgs[i],
                    "anomaly_map": current_maps[i],
        ...
        if args.visualize_samples:
            ...
            for path in tqdm(samples_dict[category]):
                ...
                ax[1].axis('off')
                ax[2].imshow(sample["gt_mask"].squeeze(), cmap='gray', interpolation='nearest')
                ...
                filename = name_without_ext.replace("/", "_").replace("\\", "_").replace(" ", "_")
