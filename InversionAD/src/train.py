...
def main(config):
    ...
    # Teraz kod będzie mógł odczytać config['meta']['seed']
    ...
    train_mean = train_dataset.mean.item() if hasattr(train_dataset.mean, 'item') else train_dataset.mean
    train_std = train_dataset.std.item() if hasattr(train_dataset.std, 'item') else train_dataset.std
    ...
    if config['backbone']['model_type'] == 'none':
        # Jeśli nie ma backbone, kształtem jest to, co wychodzi z datasetu
        # (kanały, wysokość, szerokość)
        img_size = config['data']['img_size']
        target_channels = config['diffusion']['target_channels']
        diff_in_sh = (target_channels, img_size, img_size)
    else:
        # Jeśli używasz backbone, pobierz kształt cech (np. z EfficientNet)
